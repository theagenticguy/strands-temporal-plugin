...
# =============================================================================
# DurableAgentConfig with MCP Tests
# =============================================================================


class TestDurableAgentConfigWithMCP:
    """Test DurableAgentConfig with MCP servers."""
    ...
    def test_config_with_mcp_servers(self):
        ...
        assert config.mcp_servers[0].server_id == "aws-docs"

    def test_config_with_multiple_mcp_servers(self):
        """Test config with multiple MCP servers."""
        config = DurableAgentConfig(
    ...
    def test_config_with_static_and_mcp_tools(self):
        """Test config with both static tools and MCP servers."""
        config = DurableAgentConfig(
            provider_config=BedrockProviderConfig(model_id="test"),
            tool_specs=[
                {
            ...
            tool_modules={"get_weather": "my_tools"},
            mcp_servers=[
                StdioMCPServerConfig(
                    server_id="docs",
                    command="uvx",
        ...
        assert len(config.tool_specs) == 1
        assert len(config.mcp_servers) == 1
        assert "get_weather" in config.tool_modules

    def test_config_mcp_activity_timeout(self):
        """Test MCP activity timeout configuration."""
        config = DurableAgentConfig(
            provider_config=BedrockProviderConfig(model_id="test"),
            mcp_activity_timeout=180.0,
        )
        assert config.mcp_activity_timeout == 180.0

    def test_config_get_mcp_retry_policy(self):
        """Test getting MCP retry policy."""
        config = DurableAgentConfig(
            provider_config=BedrockProviderConfig(model_id="test"),
            max_retries=5,
            initial_retry_interval_seconds=2.0,
        )
        policy = config.get_mcp_retry_policy()
        assert policy.maximum_attempts == 5

    def test_config_serialization_with_mcp(self):
        """Test config serialization with MCP servers."""
        config = DurableAgentConfig(
            provider_config=BedrockProviderConfig(model_id="test"),
            mcp_servers=[
                StdioMCPServerConfig(
        ...
        # Roundtrip
        restored = DurableAgentConfig.model_validate(data)
        assert len(restored.mcp_servers) == 1
        assert restored.mcp_servers[0].server_id == "test"
